REPLACE PROCEDURE DG_I_O_90BHR_T002.PersoonAdres_Bijwerken
...
-- DECLARES tbv JobChunkLogging.
 ...
 BEGIN
  ...
  SET JobStopDatumTijd = CURRENT_TIMESTAMP;
 ...
 SET JobBatchDatumTijd = coalesce(prmJobBatchDatumTijd,CURRENT_TIMESTAMP);
 SET JobNaam='PersoonAdres_Bijwerken';
 SET JobNaamParent = coalesce(prmJobNaamParent,null);

 SET JobStartDatumTijd= CURRENT_TIMESTAMP;
 ...
 -- Hoeveel records waren er en worden verwijdert?
 ...
 (
 SELECT 
     CAST(B110.FINR AS INTEGER) AS FiscaalNummer
    ...
    ,OREPLACE(CASE WHEN B400.POSTCODE IS NOT NULL AND TRIM(B400.POSTCODE) <> '' 
