REPLACE PROCEDURE DG_I_P_30INF_CBCI.PROC_VULLEN_BVR_VERRIJKING_INT(
    ...
    BEGIN
        ...
        DELETE
        FROM DG_I_P_30INF_CBCI.ENTITEITEN_BVR
        ...
        INSERT INTO DG_I_P_30INF_CBCI.ENTITEITEN_BVR(MESSAGEREFID, SENDINGENTITYIN, TRANSMITTINGCOUNTRY, REPORTINGENTITYTIN, RE_NAME_KEY,
        REPORTINGPERIOD, CBCBODYID, DOCREFID, "RE/CE", TIN, TIN_CLEAN, ENTITY_NAME, ETL_RUN_ID, COG_RUN_ID) -- Selecteer gegevens uit subtabel en pas regular expressies toe op de TIN om het TIN-nummer te filteren als het aan bepaalde formats voldoet
        
        SELECT A.MESSAGEREFID,
            A.SENDINGENTITYIN,
            ...
            SELECT RE.MESSAGEREFID,
                ...
                FROM DG_I_P_30INF_CBCI.REPORTING_ENTITY AS RE
                LEFT OUTER JOIN  (
                SELECT "NAME",
                    REPORTINGENTITYTIN,
                    CBCBODYID,
                    DOCREFID,
                    ETL_RUN_ID
                    FROM DG_I_P_30INF_CBCI.REPORTING_ENTITY_NAME
        ...
        SELECT B.MESSAGEREFID,
            ...
            SELECT DISTINCT CE.MESSAGEREFID,
                CE.SENDINGENTITYIN,
                ...
                CE.ETL_RUN_ID,
                par_COG_RUN_ID AS COG_RUN_ID
                FROM DG_I_P_30INF_CBCI.CBCREPORTS_CONST_ENTITIES AS CE
                LEFT OUTER JOIN (
                SELECT "NAME",
                    CBCBODYID,
                    CBCREPORTID,
                    ...
                    TIN,
                    ETL_RUN_ID
                    FROM DG_I_P_30INF_CBCI.CONST_ENTITY_NAME
        ...
        UPDATE T
        FROM DG_I_P_30INF_CBCI.ENTITEITEN_BVR AS T,
            (
        SELECT DISTINCT DT.TRANSMITTINGCOUNTRY,
            DT.MESSAGEREFID,
            DT.ETL_RUN_ID,
            DT.CBCBODYID,
            DT.Datum,
            ...
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_PERSNNPPER_FILTERED AS A
            ON DT.FINR = A.FINR
            AND DT.Datum >= A.INGDAT 
            AND DT.Datum < A.VERVDAT
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_ECONEENHEIDREL_FILTERED AS B
            ON A.FINR = B.FINR
            AND DT.Datum >= B.INGDAT 
            AND DT.Datum < B.VERVDAT
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_ECONEENHEID_FILTERED AS C
            ON B.DOSNR = C.DOSNR
            AND DT.Datum >= C.INGDAT 
            AND DT.Datum < C.VERVDAT
            LEFT OUTER JOIN DG_I_P_30INF_CBCI_INPUT.bvr_kantoor AS D
            ON C.KANTID = D.KANTID
            AND DT.Datum >= D.INGDAT
            AND DT.Datum < D.VERVDAT
            LEFT OUTER JOIN DG_I_P_30INF_CBCI_INPUT.bvr_branche AS E
            ON C.BRANCHE = E.BRANCHECD
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_VPBAKTIV_FILTERED AS F
            ON DT.FINR = F.FINR
        ...
        SELECT DISTINCT DT.TRANSMITTINGCOUNTRY,
            ...
            SELECT TRANSMITTINGCOUNTRY,
                ...
                WHERE RegExp_Similar(TIN_CLEAN, '.*\d{9}.*') = 1
                    ...
                    AND  BVR_DOSNR IS NULL) AS DT
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_PERSNNPPER_FILTERED_FINR_UNIQUE AS A
            ON DT.FINR = A.FINR
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_ECONEENHEIDREL_FILTERED_FINR_UNIQUE AS B
            ON A.FINR = B.FINR
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_ECONEENHEID_FILTERED_DOSNR_UNIQUE AS C
            ON B.DOSNR = C.DOSNR
            LEFT OUTER JOIN DG_I_P_30INF_CBCI_INPUT.bvr_kantoor AS D
            ON C.KANTID = D.KANTID
            LEFT OUTER JOIN DG_I_P_30INF_CBCI_INPUT.bvr_branche AS E
            ON C.BRANCHE = E.BRANCHECD
            LEFT OUTER JOIN DG_I_P_30INF_CBCI.BVR_VPBAKTIV_FILTERED_FINR_UNIQUE AS F
